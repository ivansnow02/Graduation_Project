...
export function calculateObjectiveMetrics(annotations: ObjectiveAnnotation[]): ObjectiveMetrics | null {
  ...
  const strategyUtteranceCount = teacherUtterances.filter((ann) => ann.teaching_strategy).length
  const strategyDensity = teacherUtteranceCount > 0 ? strategyUtteranceCount / teacherUtteranceCount : 0
  ...
  const crossDisciplinaryJumps = annotations.filter((ann) => ann.discipline_transfer === '是').length
  const finalIKTScore = teacherUtteranceCount > 0 ? crossDisciplinaryJumps / teacherUtteranceCount : 0
  ...
  const bloomProgressionRaw =
    studentCognitionLevels.length > 0
      ? Math.max(...studentCognitionLevels) - Math.min(...studentCognitionLevels)
      : 0
  ...
  const coveredIntentsCount = [...teacherIntents].filter((intent) => REQUIRED_INTENTS.has(intent)).length
  const structureCompleteness = REQUIRED_INTENTS.size > 0 ? coveredIntentsCount / REQUIRED_INTENTS.size : 0

  const l3GuidanceCount = teacherUtterances.filter((ann) => ann.teacher_guidance_level === 'L3').length
  const l3GuidanceRate = teacherUtteranceCount > 0 ? l3GuidanceCount / teacherUtteranceCount : 0

  const studentCognitionStates = studentUtterances.map((ann) => ann.student_cognition_state)
  const totalErrorCount = studentCognitionStates.filter((state) => state === '错误回答').length
  ...
  for (let i = 0; i < studentCognitionStates.length - 1; i += 1) {
    ...
    if (current === '错误回答' && (next === '高阶思考' || next === '清晰理解')) {
  ...
  return {
    StrategyDensity: { score: strategyDensity, display: formatPercent(strategyDensity) },
    StrategyVariety: { score: strategyVariety, display: formatPercent(strategyVariety) },
    ...
    StructureCompleteness: { score: structureCompleteness, display: formatPercent(structureCompleteness) },
    L3GuidanceRate: { score: l3GuidanceRate, display: formatPercent(l3GuidanceRate) },
    CognitiveCorrectionRate: { score: cognitiveCorrectionRate, display: formatPercent(cognitiveCorrectionRate) },
    TotalScore: { score: totalScore, display: totalScore.toFixed(4) },
  }
}

function buildAnnotationPrompt(dialogue: DialogueTurn[]): string {
  const dialogueText = dialogue.map((turn) => `${turn.role}：${turn.content}`).join('\n')
  return `
...
function parseAnnotations(text: string): ObjectiveAnnotation[] {
  ...
  return parsed.filter((item): item is ObjectiveAnnotation => {
    return Boolean(item && typeof item === 'object' && item.speaker && item.utterance)
  })
}
...
export function useEvaluation() {
  ...
  async function evaluateDialogue(model: ModelConfig, dialogue: DialogueTurn[], sourceHash: string): Promise<EvaluationRecord> {
    ...
    const rawText = await completeChat(model, messages, { temperature: 0, maxTokens: 5000 })
  ...
  function useEvaluationAvailability(options: {
    ...
    return computed(() => {
      return options.chatMode.value === 'single' && !options.streaming.value && Boolean(options.sourceHash.value)
    })
  }
